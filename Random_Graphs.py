...
def randomDoroMendes(n,directed,autocycles,graph_name):
    ...
    if int (n) >= 3: # Evaluating minimal nodes quantity
        ...
        for i in range(1,3): # Creating firt triangle 
            g.addNode(str(i))
        
        g.addEdge(str(1) + " -> " + str(2), str(1), str(2),NULL)
        g.addEdge(str(2) + " -> " + str(3), str(2), str(3),NULL)
        g.addEdge(str(3) + " -> " + str(3), str(3), str(1),NULL)

        for i in range(4,int (n+1)): # Creating nodes and selecting random edges to connect
            g.addNode(str(i))
            e = g.getRandEdge()
